...
/**
 * Check-out flow that REQUIRES both live location and a selfie before it will
 * submit — the mirror of CheckInModal, same three steps:
 *   1. Grant location  -> we read GPS coordinates
 *   2. Capture a photo -> live camera, snap a frame
 *   3. Confirm         -> uploads the photo to /api/files, then POSTs check-out
 *
 * If either location or camera is denied, check-out is blocked (matching the
 * server-side requirement in AttendanceService.checkOut).
 */
export default function CheckOutModal({
  ...
  const handleConfirm = async () => {
    ...
    setSubmitting(true);
    try {
      await attendanceService.checkOutWithProof(
        employeeId,
        coords.latitude,
        coords.longitude,
        photo,
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between border-t border-slate-100 pt-4">
          <p className="text-xs text-slate-400">
            Both location and photo are required to check out.
          </p>
